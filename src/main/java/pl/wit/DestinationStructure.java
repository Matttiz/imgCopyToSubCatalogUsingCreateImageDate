...
public class DestinationStructure extends LinkedHashSet<String> {
	...
	/**
	 *
	 * @param filesPathAndCreateFoldersList lista ścieżek wraz z informacją o
	 *                                      folderze, do którego należy skopiować
	 *                                      plik
	 * @param destination                   ścieżka do folderu, w którym mają zostać
	 *                                      utworzone foldery i przekopiowane pliki
	 ...
	 * 
	 * @param destinationStructure obiekt zawierający informację o tym gdzie jest
	 *                             folder docelowy oraz jakie foldery powinn zostać
	 *                             utworzone
	 */
	...
	void createDestinationFolders() {
		...
		for (String item : names) {
			dateFolder = new FileExtended(this.destination + FileExtended.separatorChar + item);
	...
	public ArrayList<Boolean> exists() {
		...
		for (String item : names) {
			dateFolder = new FileExtended(
					this.destination + FileExtended.separatorChar + item + FileExtended.separatorChar);
	...
	/**
	 * Metoda zwracająca ścieżkę absolutną do folderu wraz z inforacją czy folder
	 * istnieje
	 * 
	...
	public LinkedHashMap<String, Boolean> isFolderExist() {
		...
		while (it.hasNext()) {
			folder = it.next();
			dateFolder = new FileExtended(this.destination + FileExtended.separatorChar + folder);
	...
	/**
	 * Metoda wylicza wartość pliku o w folderze o najwyższej wartośli liczbowej w
	 * przypadku gdyby folder docelowy istnial przed rozpoczęciem działania programu
	 * 
	 * @return LinkedHashMap String,Integer nazwa folderu wraz z informacją o
	 *         najwyższym numerze pliku się w nim znajdującym
	 * @throws IOException może zostać wygenerowany przez metodę wewnętrzną
	 *                     getHighestFileInFolder
	 */
	public LinkedHashMap<String, Integer> getFolderAndHighestFileWithExtenionMap() throws IOException {
		...
		for (String folder : folderToCheckExistedFiles) {
			if (isFolderExistMap.get(folder)) {
				highestFileNumber = FileExtended
						.getHighestFileInFolder(destination + FileExtended.separatorChar + folder, this.extension);
	...
	public LinkedHashMap<String, Integer> getFolderAndHighestFileWithExtenionMap(String extension) throws IOException {
		...
		for (String folder : folderToCheckExistedFiles) {
			if (isFolderExistMap.get(folder)) {
				highestFileNumber = FileExtended
						.getHighestFileInFolder(destination + FileExtended.separatorChar + folder, extension);
				if (highestFileNumber == -1) {
					highestFileNumber = 0;
				}
			} else {
